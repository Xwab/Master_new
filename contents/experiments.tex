...
\paragraph{Llama 3.1 8B Instruct} 该模型来自 Meta 的 Llama 3.1 系列，拥有约 80 亿参数，采用高效的 transformer decoder 架构和混合语料训练，强调英语及多语对话能力。Instruct 版本通过 RLHF 与安全过滤进一步对齐，能够提供稳定的指令遵循表现，在推理和生成任务中兼具质量与速度。其较紧凑的参数规模使得 KV 压缩潜力大，适合作为评估英语场景的代表性模型。

\paragraph{Qwen2.5 7B Instruct} Qwen2.5 是阿里提出的新一代中文主导模型，7B Instruct 版本在大规模中文语料和跨语数据上训练，并结合指令调优、工具调用能力和安全约束。模型在中文问答、对话、知识检索等方面表现突出，同时保持对英文任务的兼容性。其架构与 Llama 系列类似，但在分词方式、训练语料和对齐策略上具有明显差异。

在实验中，我们保持两种模型的原始权重不变，只在推理阶段施加不同的 KV 压缩与量化策略，然后统一在上述数据集上评估困惑度、准确率以及 LongBench 的多个指标，以验证我们方法在不同语种、不同训练风格下的通用性与稳定性。