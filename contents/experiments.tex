...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcccccccc}
        \toprule
        \textbf{模型 / 压缩方式} & \textbf{BoolQ} & \textbf{HellaSwag} & \textbf{OpenBookQA} & \textbf{ARC-C} & \textbf{ARC-E} & \textbf{Winogrande} & \textbf{PIQA} & \textbf{CEval} \\
        \midrule
        原模型 (Llama3.1-8B-Instruct) &  &  &  &  &  &  &  &  \\
        SVD &  &  &  &  &  &  &  &  \\
        ASVD &  &  &  &  &  &  &  &  \\
        Palu &  &  &  &  &  &  &  &  \\
        \textbf{Ours} &  &  &  &  &  &  &  &  \\
...
\begin{table}[htbp]
    \centering
    \caption{Llama3.1-8B-Instruct 在语言建模数据集上的困惑度 (PPL)}
    \label{tab:lm-ppl}
    \begin{tabular}{lcc}
        \toprule
        \textbf{模型 / 压缩方式} & \textbf{WikiText-2} & \textbf{PTB} \\
        \midrule
        原模型 (Llama3.1-8B-Instruct) & 7.21 & 12.33 \\
        SVD & 9.93 & 17.03 \\
        ASVD & 9.16 & 15.62 \\
        Palu & 8.94 & 15.13 \\
        \textbf{Ours} & \textbf{8.04} & \textbf{14.14} \\
        \bottomrule
    \end{tabular}
\end{table}
\begin{table}[htbp]
    ...
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Task} & \textbf{Task Type} & \textbf{Eval Metric} & \textbf{Avg len} & \textbf{Language} & \textbf{\#Sample} \\
        \midrule
...
\section{实验数据集与评估指标}
为了全面评估所提出压缩策略对模型能力的影响，本章按任务类型将实验划分为三个层面：首先在语言建模基准上计算困惑度，量化压缩对基础序列建模质量的影响；随后在多样化的零样本推理任务上以准确率衡量知识与常识推理的保持程度；最后在 LongBench 长上下文基准上覆盖 21 个子任务，使用 F1、Rouge-L、Accuracy、Edit Similarity 等指标考察模型在文本生成、问答、分类与代码补全等复杂场景中的表现。下面依次介绍各类数据集与评估方式。
\subsection{语言建模与困惑度}
...
LongBench 统一采用以下指标：
\begin{itemize}
    \item \textbf{F1}：针对抽取式问答或文本匹配场景，先逐 token（或逐字）计算预测答案与参考答案之间的精确率（precision）和召回率（recall），再取二者的调和平均。相比单纯的精确率或召回率，F1 能同时考虑答案是否覆盖了所有关键 span 以及是否引入多余内容，因此更适合衡量模型在长上下文问答中的综合表现。
...
\section{实验模型设置}
为了验证压缩策略在不同架构与训练范式下的适应性，我们选择两条主流指令微调模型作为主体评测对象，分别代表西语系和中文系的典型开源大模型。

\paragraph{Llama 3.1 8B Instruct} Llama 3.1 是 Meta 最新一代的开源基础模型系列，8B 版本在参数规模与推理成本之间取得平衡，采用标准的 decoder-only Transformer 架构，搭配持续优化的自回归训练策略与高质量混合语料。官方 Instruct 版本对原始模型进行了 RLHF、拒答/安全微调和风格调优，使其在指令遵循、对话问答、摘要生成等任务上保持稳定、可控的输出。该模型具备较长的上下文窗口和良好的多语言支持能力，也是开源社区常用的实验基准之一。

\paragraph{Qwen2.5 7B Instruct} Qwen2.5 延续了 Qwen 系列的 Transformer 架构，7B Instruct 版本在大规模跨领域语料（包括技术文档、对话语料、知识图谱扩展文本等）上训练，并结合高质量的指令集做监督微调与 RLHF。模型支持多语言混合分词，内置工具调用与函数描述学习能力，在问答、对话、长文本任务上表现突出。官方提供的 Instruct 版本还包含安全过滤与格式化输出能力，使其在指令式评测中有更好的稳定性。

在实验中，我们保持以上两种模型的原始权重与推理配置不变，只在推理阶段施加不同的 KV 压缩与量化策略，然后统一在上述数据集上评估困惑度、准确率以及 LongBench 指标，以验证所提方法在不同预训练语料与对齐范式下的通用性。

\section{实验设置}
所有实验均在配备 NVIDIA L20（48GB 显存）的服务器上完成。为了保证各子模块的对比公平，我们使用统一的推理参数（最大生成步数、温度等），并仅在 KV 压缩与量化策略上做变化。具体到本论文提出的两阶段方法：
\begin{itemize}
    \item 在 \texttt{methdology1.tex} 中用于构建 scaling 矩阵的 calibration 集，英文部分采样自 BoolQ 训练集的 512 个样本，中文部分采样自 DuReader 训练集的 128 个样本。二者覆盖“是/否”问答和多文档中文 QA，有助于估计激活/注意力感知矩阵所需的统计量。
    \item 在 \texttt{methdology2.tex} 的层间贪心 rank 分配算法中，为了提升效率，我们为英文模型选取了 PIQA 训练集的 32 个样本，为中文模型选取了 VCSUM 的 16 个样本作为 calibration 集。由于 rank 变化敏感性与误差累积更偏向于模型自身特性，即便样本量较小也能稳定反映趋势。
    \item Rank 传递的超参数设置为：最大迭代步数 20，每次 rank 调整（Key 与 Value 独立计算）的学习率 $\eta = 0.4$，并在所有实验中固定。若达到最大步数或连续多轮无提升，则按照第二章描述提前停止。
\end{itemize}
在上述设置下，我们对 Llama 3.1 8B Instruct 与 Qwen2.5 7B Instruct 分别运行低秩、低秩+量化、混合精度等策略，并在全部数据集与指标上进行评估。