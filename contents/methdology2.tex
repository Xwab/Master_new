...
对于每个样本 $x\in\mathcal{D}_\text{cal}$，我们逐层运行当前秩分配下的压缩模型，记录每层输出 $h_l$ 作为下一层输入。当前向到第 $l $ 层时，复制出 $l_k$、$l_v$ 两个变体：$l_k$ 保持 Value 的秩不变，但将 $S_kW_k$ 恢复到完整的 $d$ 维；$l_v$ 则保持 Key 的秩不变，还原 $[S_{v,1}W_{v,1},\ldots,S_{v,G}W_{v,G}]$ 的全部 $d$ 个秩。如此对每层都构造“仅 Key 不压缩”与“仅 Value 不压缩”的版本，就相当于得到了 $2L+1$ 个模型（包含一个当前秩配置下的压缩模型）。我们将这 $2L$ 个“单层解压”模型的输出与压缩模型输出进行 L2 对比，差值越大说明该层压缩对最终结果影响越显著，需要分配更多秩；差值小的层则可以回收一部分秩。但若 naïve 地为每个样本独立运行 $2L+1$ 次前向，代价极高，因此我们提出了动态规划式的缓存方案，在一次前向中同时得到所有分支的输出。

记第 $l$ 层压缩映射为 $F_l$，仅 Key/Value 解除压缩的版本为 $F_{l,k}$、$F_{l,v}$，其输出分别为 $h_l$、$h_{l,k}$、$h_{l,v}$。算法维护一个队列 $\mathcal{Q}$，初始时把 $h_{0,k}$、$h_{0,v}$ 入队。当前向至第 1 层时，先计算压缩输出 $h_1=F_1(h_0)$，再将队首的 $h_{0,k}$、$h_{0,v}$ 依次输入 $F_1$ 得到新的 $h_{0,k}$、$h_{0,v}$，并回插队尾；同时，把 $h_0$ 分别送入 $F_{1,k}$、$F_{1,v}$ 得到 $h_{1,k}$、$h_{1,v}$，追加到队列末尾。对任意 $l$，队列在进入第 $l$ 层前的状态为 $[h_{0,k},h_{0,v},\ldots,h_{l-1,k},h_{l-1,v}]$，我们依次将这些元素出队，输入 $F_l$ 得到经过第 $l$ 层后的值，再入队相应的更新；随后计算 $h_{l,k}=F_{l,k}(h_{l-1})$、$h_{l,v}=F_{l,v}(h_{l-1})$ 并入队。最终，在第 $L$ 层完成后，队列中包含 $2L$ 个“单层解压”输出，与压缩模型输出一起构成 $2L+1$ 组 logits。

动态规划伪代码如下：

\begin{center}
\begin{minipage}{0.95\linewidth}
\textbf{算法 1：单次前向生成 $2L+1$ 组输出}

\textbf{Input:} 样本 $x$，层数 $L$，映射 $F_l,F_{l,k},F_{l,v}$ \\
\textbf{Output:} $\{y^{\text{full}}, y^{(k,l)}, y^{(v,l)}\}_{l=0}^{L-1}$

\begin{enumerate}
    \item $\mathcal{Q}\gets\varnothing$；$h_0\gets F_0(x)$；$h_{0,k}\gets F_{0,k}(x)$、$h_{0,v}\gets F_{0,v}(x)$，并入队。
    \item \textbf{for} $l=1$ \textbf{to} $L-1$ \textbf{do}
    \begin{enumerate}
        \item $h_l \gets F_l(h_{l-1})$。
        \item \textbf{for each} $(h_{i,k},h_{i,v})$ in $\mathcal{Q}$ \textbf{do}\\
        \quad $h_{i,k} \gets F_l(h_{i,k})$；$h_{i,v} \gets F_l(h_{i,v})$；将更新后的 $(h_{i,k},h_{i,v})$ 回插队尾。
        \item 计算 $h_{l,k}\gets F_{l,k}(h_{l-1})$、$h_{l,v}\gets F_{l,v}(h_{l-1})$，并入队。
    \end{enumerate}
...
通过该流程，一次前向即可获取所有层的“单层解压”输出，用于后续 L2 误差统计与秩重分配。
