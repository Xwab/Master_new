...
对于每个样本 $x\in\mathcal{D}_\text{cal}$，我们首先运行 full-rank 模型得到基准 logits $y^{\text{full}}(x)$。随后对每一层单独执行“只压缩该层、其他层保持当前秩”的前向，利用前述缓存实现类似动态规划的加速：一旦第 $l$ 层 full-rank 输出被记录，后续所有层都可以在“全秩馈线”与“压缩馈线”之间切换，从而在一次校准运行中同时得到
\[
\Delta_{k,l}(x)=\bigl\|y^{\text{full}}(x)-y^{(k,l)}(x)\bigr\|_2,\qquad
\Delta_{v,l}(x)=\bigl\|y^{\text{full}}(x)-y^{(v,l)}(x)\bigr\|_2,
\]
其中 $y^{(k,l)}(x)$ 表示仅第 $l$ 层 Key 压缩时的输出，$y^{(v,l)}(x)$ 定义类似。由于校准集规模通常仅占真实推理负载的 0.1\%–1\%，这类额外前向对总体成本影响可控，却能提供关于误差传播的高置信度统计。将 $\Delta_{k,l}(x)$、$\Delta_{v,l}(x)$ 在校准集上取均值得到 $\overline{\Delta}_{k,l}$、$\overline{\Delta}_{v,l}$，它们直接反映了各层的误差累积风险；若需要更加稳健，还可附加标准差或分位数指标用于检测离群样本。
