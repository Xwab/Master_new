...
\Return $y^{\text{full}}, \{y^{(k,l)}, y^{(v,l)}\}_{l=0}^{L-1}$；
\end{algorithm}
...
\While{$t < T_{\max}$ \KwAnd $c < K$}{
    ...
    $\delta_k \gets \mathrm{clip}(\eta(\alpha_{\ell_{k,\text{max}}} + \alpha_{\ell_{k,\text{min}}}), \delta_{\min}, \delta_{\max})$；\\
    $\delta_v \gets \mathrm{clip}(\eta(\alpha_{\ell_{v,\text{max}}} + \alpha_{\ell_{v,\text{min}}}), \delta_{\min}, \delta_{\max})$；\\
