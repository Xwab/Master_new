...
设模型共有 $L$ 个注意力层，每层在 GQA 架构下含有 $G$ 个 KV 组，每组的 Key/Value 维度均为 $d$，一次推理写入缓存的序列数为 $N$，缓存长度为 $T$。在不压缩的情况下，单层写入 KV 缓存的体积为
\begin{equation}
    \label{eq:kv-cost}
    \mathcal{C}_l^{\text{KV}} = 2 N \cdot T \cdot G \cdot d,
\end{equation}
于是模型所有层合计的 KV 体积是 $\mathcal{C}^{\text{KV}}=\sum_{l=1}^{L}\mathcal{C}_l^{\text{KV}} = 2 N T L G d$。由于变换后的 $M_{k,l}$、$M_{v,l}$ 与其 SVD 中间表示在不裁剪秩时的特征维度一致，都为 $d$，因此在归一化到单个 token-组对之后，$\mathcal{C}^{\text{KV}}/(N T G)=2Ld$ 可视为模型可使用的最大秩总数。若全模型的目标压缩率为 $\rho \in (0,1]$，则所有层保留下来的秩之和必须满足
\[
    \sum_{l=1}^{L}\bigl(r_{k,l}+r_{v,l}\bigr) \le \rho \cdot \frac{\mathcal{C}^{\text{KV}}}{N T G}
    = \rho \cdot 2 L d.
\]

在这一约束下，跨层 rank 调度可以转写为一个归一化误差最小化问题。设我们对 $M_{k,l}$ 与 $M_{v,l}$ 分别保留 $r_{k,l}$、$r_{v,l}$ 个奇异值，$M_{k,l}^{(r_{k,l})}$、$M_{v,l}^{(r_{v,l})}$ 表示对应的最优秩-$r$ 近似，则初始化阶段希望求解
\begin{equation}
    \label{eq:opt-init}
    ...
    \right),
\end{equation}
其中每个误差项都被原矩阵的 $\mathrm{F}$ 范数平方归一化：不同层的参数规模和激活统计差异会导致 $\|M_{k,l}\|_F^2$、$\|M_{v,l}\|_F^2$ 相差数倍，如果直接比较未归一化的能量，偏大层的尾部奇异值会因为绝对能量高而被优先保留，违背“同等保留比例”这一目标。通过除以本层的总能量，我们让所有层、所有奇异值都在相同尺度上参与排序，从而实现公平的 rank 分配。式 \eqref{eq:opt-init} 的约束仍为
\begin{equation}
