...
对于每个样本 $x\in\mathcal{D}_\text{cal}$，我们首先运行 full-rank 前向，记录每层的激活 $H_l^{\text{full}}(x)$ 以及 Key/Value 投影输出 $H_l^{\text{full}}(x)S_{k,l}W_{k,l}$、$H_l^{\text{full}}(x)S_{v,l}W_{v,l}$。随后使用当前的秩配置重新前向，但沿途携带 full-rank 缓存并在每层计算压缩差异：
\[
\Delta_{k,l}(x)=\bigl\|H_l^{\text{full}}(x)S_{k,l}W_{k,l}-H_l^{\text{full}}(x)S_{k,l}P_{k,l}Q_{k,l}\bigr\|_2,
\]
\[
\Delta_{v,l}(x)=\bigl\|H_l^{\text{full}}(x)S_{v,l}W_{v,l}-H_l^{\text{full}}(x)S_{v,l}P_{v,l}Q_{v,l}\bigr\|_2.
\]
为避免重复推理，我们采用类似动态规划的缓存方式：一旦第 $l$ 层 full-rank 输出被记录，后续所有层都会同时携带“全秩馈线”与“压缩馈线”，从而可以在一次校准运行中同时获得“仅第 $l$ 层压缩”对最终 logits 的影响。将 $\Delta_{k,l}(x)$、$\Delta_{v,l}(x)$ 在校准集上取均值得到 $\overline{\Delta}_{k,l}$、$\overline{\Delta}_{v,l}$，它们直接反映了各层的误差累积风险。
...
\]
其中 $\eta$ 为学习率。通过 $r_{\ell_\text{max}}\leftarrow r_{\ell_\text{max}}+\delta$、$r_{\ell_\text{min}}\leftarrow r_{\ell_\text{min}}-\delta$ 完成一次 rank 传递，并在新的秩配置下重新运行校准，若连续 $K$ 轮迭代都未降低“压缩模型 vs.~full 模型”的 L2 误差，则回滚至最佳秩配置并终止。
