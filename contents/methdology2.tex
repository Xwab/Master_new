...
跨层 rank 调度的起点是弄清楚各层原始的 KV 规模。设模型共有 $L$ 个注意力层，第 $l$ 层在 GQA 架构下含有 $G_l$ 个 KV 组，Key 与 Value 的特征维度统一记作 $d$，推理缓存长度为 $T$。不压缩时写入 KV 缓存的体积是
\begin{equation}
    \label{eq:kv-cost}
    \mathcal{C}_l^{\text{KV}} = T \cdot G_l \cdot 2 d,
\end{equation}
也是该层可使用的最大 rank。若全模型目标压缩率为 $\rho \in (0,1]$，则剩余的 rank 总量需满足 $\sum_l \bigl(r_{k,l}+r_{v,l}\bigr) \approx \rho \sum_l 2 d$。

为了在这个约束下给每层的 Key/Value 分配初值，我们分别对 $M_{k,l}$ 与 $M_{v,l}$ 做 SVD。记其奇异值为 $\{\sigma_{k,l,i}\}_{i=1}^{d}$、$\{\sigma_{v,l,i}\}_{i=1}^{d}$。第 $i$ 个秩的相对能量占比分别为
\begin{equation}
...
\begin{equation}
    C_{k,l}=T \cdot G_l \cdot d,\qquad
    C_{v,l}=T \cdot G_l \cdot d,\qquad
