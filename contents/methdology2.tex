...
\]
为避免重复推理，我们采用类似动态规划的缓存方式：一旦第 $l$ 层 full-rank 输出被记录，后续所有层都会同时携带“全秩馈线”与“压缩馈线”，从而可以在一次校准运行中同时获得“仅第 $l$ 层压缩”对最终 logits 的影响。由于校准集规模通常仅占真实推理负载的 0.1\%–1\%，这类额外前向对总体成本影响可控，却能提供关于误差传播的高置信度统计。将 $\Delta_{k,l}(x)$、$\Delta_{v,l}(x)$ 在校准集上取均值得到 $\overline{\Delta}_{k,l}$、$\overline{\Delta}_{v,l}$，它们直接反映了各层的误差累积风险；若需要更加稳健，可以附加标准差或分位数指标用于检测离群样本。
