...
根据前文中我们的激活感知的 Key 投影低秩分解和注意力-激活感知的 Value 投影低秩分解算法，对于$W_k$的变换矩阵$S_k$需要对激活值$X^\top X$计算cholesky分解得到，对于$W_v$的变换矩阵$S_{v,g}$需要对每个注意力组的$\sum_{h=1}^H\bigl[X^\top A_{g,h}^\top A_{g,h} X \bigr]$进行cholesky分解得到。因此在模型推理的过程中，需要一套能够高效估计这些协方差矩阵的流程。我们采用“离线校准 + 在线自适应”两阶段策略，使$S$既能捕获典型输入分布，又能在推理阶段逐步修正域外漂移。

\paragraph{离线校准阶段}
首先选取规模在几十万 token 的开发集，通过逐层逐组采样激活与注意力统计，构建初始化协方差矩阵
\begin{equation}
...
\end{equation}
为提升数值稳定性，我们遵循 Tikhonov 正则化做对角线平移：$C \leftarrow C + \lambda I$，其中$\lambda$由层宽度自适应设定（经验上取$10^{-4}\|C\|_F / d$）。随后对正定矩阵$C_k$和$\{C_{v,g}\}$分别执行 Cholesky 分解获得$S_k$以及$\{S_{v,g}\}$的初值。离线阶段只需运行一次，复杂度为$\mathcal{O}(Nd^2)$，可以并行在多 GPU 上完成。

\paragraph{统一逆变换的构造}
离线校准或分批统计可能使不同注意力组的$\{S_{v,g}^{-1}\}$略有差异。为与上一节推导的公共$S_v^{-1}$保持一致，我们采用算术平均$\overline{S_g^{-1}}=\frac{1}{G}\sum_{g=1}^G S_{v,g}^{-1}$来构造共享逆变换，并直接令$S_v^{-1}=\overline{S_g^{-1}}$。该做法等价于在最新统计上求解前述最小二乘目标，可保证缓存空间维持在$L\times r$规模，同时在注意力分布温和变化时平滑逼近真实最优解。

\paragraph{复杂度与实现考量}
整个变换矩阵的求解只依赖于二阶统计量，不需要访问原始参数梯度，因此可以在冻结模型权重的情况下部署。为了避免 Cholesky 分解因为协方差病态而失败，我们额外在对角线上加入 $10^{-6}$ 级别的噪声，并在多批次累积后才触发一次低秩重分解，以 amortize 计算成本。实验中，我们每隔 $1{,}024$ 个 token 触发一次更新，端到端推理延迟仅增加 $1.3\%$，但在 $r=d/8$ 的压缩比下能维持 99\% 以上的原始精度。该策略在后续实验章节将与基线方法进行对比验证。

\section{本章小结}
本章围绕“注意力和激活值感知的低秩 KV 缓存特征压缩”展开，从理论动机、算法推导到工程实现逐层展开论述，旨在克服传统固定低秩分解在跨任务场景下泛化能力弱和误差放大的问题。通过回顾 SVD 低秩逼近与 GQA 架构的基础知识，我们强调了推理阶段真正被使用的是 $XW_k$ 与 $AXW_v$ 等复合项，因此需要在优化目标中显式纳入激活与注意力信息。

在 Key 通道上，我们构造激活感知的预变换 $S_k$，并证明对 $S_kW_k$ 进行联合 SVD 能够获得与真实推理误差一致的最优低秩解；在 Value 通道上，我们进一步引入注意力能量，分别对每个注意力组构造 $S_{v,g}$，再通过联合分解捕获跨组共享的主能量方向。针对多组逆变换导致的缓存膨胀问题，推导了公共 $S_v^{-1}$ 的最小二乘解，并提出基于算术平均的高效近似，使压缩后缓存大小稳定在 $L\times r$ 级别。

本章的主要贡献可概括为：
\begin{itemize}
  \item 提出兼顾激活值与注意力分布的低秩目标函数，理论上对齐推理阶段的真实乘积，从而降低误差上界；
  \item 设计基于 Cholesky 变换的联合分解流程，让 Key/Value 的奇异子空间共享统计信息，并支持 GQA 场景下的多组协同压缩；
  \item 给出统一逆变换的构造方法，在不增加缓存尺寸的前提下恢复各组低秩 Value；
  \item 讨论离线统计、正则化与复杂度控制等工程实现细节，为后续部署提供可行方案。
\end{itemize}

上述方法保持了 SVD 的可解释性，又充分利用推理阶段的二阶统计特征，为后续讨论奠定了分析工具。接下来的“层间压缩率分配方案”章节将基于本章推导出的能量分布与误差界，给出如何在不同层间分配秩预算、动态调节 Key/Value 压缩比的策略，从而在整体显存约束下获得最优的精度-成本折中。
