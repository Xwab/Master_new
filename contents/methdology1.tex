...
\paragraph{在线自适应阶段}
为了避免静态统计在真实推理任务中失效，我们在 KV 缓存写入时维护指数滑动协方差估计。设第$t$个请求产生的激活样本为$X^{(t)}$，则更新规则为
\begin{equation}
  \widetilde{C}_k^{(t)} = (1-\alpha)\widetilde{C}_k^{(t-1)} + \alpha X^{(t)\top} X^{(t)}, \qquad
  \widetilde{C}_{v,g}^{(t)} = (1-\alpha)\widetilde{C}_{v,g}^{(t-1)} + \alpha \sum_{h=1}^{H} X^{(t)\top} A_{g,h}^{(t)\top} A_{g,h}^{(t)} X^{(t)},
  \label{eq:ema_cov}
\end{equation}
其中$\alpha$为平滑系数（通常设为$10^{-3}\sim10^{-2}$），确保协方差在少量新样本下不会产生剧烈震荡。每当更新后的矩阵与旧矩阵差异超过阈值$\delta$时，重新执行一次增量 Cholesky 分解，以获得新的$S_k$或$S_{v,g}$。由于 Cholesky 的下三角矩阵可以通过 rank-one 更新快速调整，我们仅需 $\mathcal{O}(d^2)$ 的开销即可完成一次自适应。

\paragraph{统一逆变换的构造}
在线阶段得到的$\{S_{v,g}^{-1}\}$会持续改变，为了与上一节推导的公共$S_v^{-1}$保持一致，我们在每次刷新局部矩阵后，同步更新其算术平均$\overline{S_g^{-1}}=\frac{1}{G}\sum_{g=1}^G S_{v,g}^{-1}$，并直接令$S_v^{-1}=\overline{S_g^{-1}}$。该近似等价于在滑动窗口内求解式~\eqref{eq:ema_cov}对应的最小二乘问题，可保证缓存空间始终维持在$L\times r$规模，同时在注意力分布发生缓慢漂移时渐进逼近真实最优解。
