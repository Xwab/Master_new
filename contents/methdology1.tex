...
根据前文中我们的激活感知的 Key 投影低秩分解和注意力-激活感知的 Value 投影低秩分解算法，对于$W_k$的变换矩阵$S_k$需要对激活值$X^\top X$计算cholesky分解得到，对于$W_v$的变换矩阵$S_{v,g}$需要对每个注意力组的$\sum_{h=1}^H\bigl[X^\top A_{g,h}^\top A_{g,h} X \bigr]$进行cholesky分解得到。因此在模型推理的过程中，需要
