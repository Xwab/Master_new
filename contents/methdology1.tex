...
\section{本章小结}
本章围绕“注意力和激活值感知的低秩 KV 缓存特征压缩”展开，首先回顾了 SVD 与 GQA 的理论背景，并指出直接对参数矩阵做低秩近似无法充分反映推理阶段的真实运算。针对 Key，我们通过引入激活协方差的 Cholesky 变换，将优化目标转化为对 $S_kW_k$ 的联合 SVD，显式对齐实际的 $XW_k$ 乘积；针对 Value，我们进一步把注意力分数纳入目标函数，对每个分组构造独立的 $S_{v,g}$ 并联合分解，使保留的奇异子空间同时对齐激活与注意力能量。为解决多组逆变换带来的缓存膨胀，提出以算术平均近似统一 $S_v^{-1}$ 的策略，并给出了变换矩阵在离线阶段的高效求解与工程实现要点。综合来看，该方法在保持 SVD 可解释性的同时，更贴近推理时的能量分布，为后续章节的实验验证奠定理论基础。
